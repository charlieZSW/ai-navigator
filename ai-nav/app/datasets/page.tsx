...
import { datasets, datasetCategories } from '../data/datasets';
...
export default function DatasetsPage() {
  ...
  // 计算当前应该显示的数据集
  const visibleDatasets = filteredDatasets.slice(0, currentPage * ITEMS_PER_PAGE);
  const hasMoreItems = visibleDatasets.length < filteredDatasets.length;
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="mb-12 animate-fade-in">
          <h1 className="text-3xl font-bold text-slate-800 mb-4">AI Datasets</h1>
        ...
        <div className="mb-8">
          <div className="relative max-w-xl">
            <input
              type="text"
              placeholder="Search datasets, descriptions, or tags..."
        ...
        <div className="mb-8 text-slate-600">
          Found <span className="text-slate-800 font-semibold">{filteredDatasets.length}</span> datasets
        ...
        {filteredDatasets.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {visibleDatasets.map((dataset, index) => (
              <div key={dataset.id} className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md hover:border-blue-300 transition-all transform hover:-translate-y-1 flex flex-col h-full">
                <div className="p-5 flex-grow">
                  <Link 
                    href={formatLink(dataset.link)}
                    target="_blank" 
                  ...
                  <div>
                    <p className={`text-slate-600 text-sm mb-1 ${expandedDescriptions[dataset.id] ? '' : 'line-clamp-2'}`}>
                      {dataset.description}
                    </p>
                    ...
                    {dataset.description.length > 100 && (
                      ...
                      >
                        {expandedDescriptions[dataset.id] ? 'Collapse' : 'View Full Description'}
                      </button>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    ...
                    {dataset.format && (
                      <span className="text-xs px-2 py-1 bg-teal-50 text-teal-600 rounded-full whitespace-nowrap">
                        {dataset.format}
                      </span>
                    )}
                    
                    {dataset.size && (
                      <span className="text-xs px-2 py-1 bg-violet-50 text-violet-600 rounded-full whitespace-nowrap">
                        {dataset.size}
                      </span>
                    )}
                  </div>
          ...
          <div className="bg-white border border-slate-200 rounded-lg p-8 text-center">
            <p className="text-slate-600 mb-2">No matching datasets found</p>
        ...
        {filteredDatasets.length > 0 && (
          ...
          >
            {hasMoreItems ? (
              ...
              <span className="text-slate-500 text-sm">All {filteredDatasets.length} datasets loaded</span>
            )}
          </div>
        )}
      </div>
